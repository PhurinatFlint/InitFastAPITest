...
"""
Create user via APIs example. No longer uses
"""
...
async def get_current_user(request: Request):
    try:
        token = request.cookies.get("access_token") # Get token by cookies which given through APIs instead
        if token is None:
            return None
        ...
        if username is None or user_id is None:
            logout(request=request) # Automatically logout and delete cookies
            #return None
            #raise get_user_exception()
        return {"username":username, "id":user_id}
    except JWTError:
        raise HTTPException(status_code=404, detail="Get current user not found")
...
async def logout(request: Request):
    ...
    response.delete_cookie(key="access_token") # Delete cookies when logout
    return response
...
"""
These exceptions are not been used anymore. It's for exception via REST API purpose. !!!! but it's necessary.
"""
## Exceptions
